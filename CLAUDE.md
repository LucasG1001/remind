...
- **`habits`** — `id`, `name`, `selected_days INTEGER[]` (0–6), `icon`, `target_count`, `duration_minutes` (NULL = sem timer; obrigatório no `PUT`, como `targetCount`), `position`, timestamps. Sequência/nível são recalculados no cliente (não persistidos). `GET /api/habits` devolve o histórico completo de conclusões, sem janela de data — é o que sustenta a visão de ano.
