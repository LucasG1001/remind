...
- **`lib/validation.ts`** — `requireUuid`, `parseBody(res, schema, body)`, `respondValidationError` e as regex `DATE_RE`/`TIME_RE`, compartilhados pelos controllers. **`lib/timeWindow.ts`** — funções puras da janela de horário do hábito (`checkTimeWindow`, `toHhMm`, `addMinutesToTime`…), testadas e espelhadas em `frontend/src/utils/timeWindow.ts`. **`lib/sqlUpdate.ts`** — `buildUpdateSet` + `nextPositionSql`.
...
- **Habits**: `models/habitModel.ts` (inclui conclusões + `CompletionLockedError`; o mapper `toHabit` é o único ponto que corta o `HH:MM:SS` do pg para `HH:MM`), `controllers/habitController.ts`. Sem scheduler/notificações.
